...
def get_fast_links(query):
    """10. Naprawiona wyszukiwarka (poprawne nagłówki + timeout)"""
    query = query.strip()
    url = "https://fitgirl-repacks.site/"
    try:
        # Przywrócona oryginalna logika nagłówków (User-Agent) ze startu + timeout=15
        with httpx.Client(http2=True, timeout=15.0) as client:
            response = client.get(url, params={'s': query}, headers={
...
<head>
    ...
    <style>
        ...
        .section-toggle { background: #1e1e28; border: 1px solid #2a2a38; color: #9aa; padding: 10px 18px; border-radius: 10px; font-size: 0.85rem; font-weight: 600; cursor: pointer; width: 100%; text-align: left; margin-top: 20px; transition: 0.2s; }
        ...
        .comment-date { color: #444; font-size: 0.75rem; margin-left: 10px; }
        .comment-text { color: #bbb; margin-top: 6px; font-size: 0.87rem; line-height: 1.6; }
        ...
        .comment-reply .comment-user { color: #666; }
        .comment-reply .comment-text { color: #666; }
        .btn-comments { background: #1e1e28; border: 1px solid #2a2a38; color: #9aa; padding: 10px 18px; border-radius: 10px; font-size: 0.85rem; font-weight: 600; cursor: pointer; width: 100%; margin-top: 15px; transition: 0.2s; }
